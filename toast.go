...
$template = @"
<toast scenario="{{.Scenario}}" activationType="{{.ActivationType}}" launch="{{.ActivationArguments}}" duration="{{.Duration}}">
	<visual>
        <binding template="ToastGeneric">
			...
			{{if or .ProgressBar.Title .ProgressBar.Value .ProgressBar.ValueStringOverride .ProgressBar.Status}}
			<progress
				title="{{.ProgressBar.Title}}"
				value="{{.ProgressBar.Value}}"
				valueStringOverride="{{.ProgressBar.ValueStringOverride}}"
    ...
    {{if .Actions}}
    <actions>
		{{if or .Input.Id .Input.Type .Input.DefaultInput}}
		<input id="{{.Input.Id}}" type="{{.Input.Type}}" defaultInput="{{.Input.DefaultInput}}">
			{{range $select := .Input.Selections}}
			<selection id="{{$select.Id}}" content="{{$select.Content}}"/>
		  	{{end}}
		</input>
		{{end}}

        {{range .Actions}}
        <action activationType="{{.Type}}" content="{{.Label}}" arguments="{{.Arguments}}" hin-inputId="{{.HintInputId}}" />
        {{end}}
    </actions>
    {{end}}
</toast>
...
// See https://docs.microsoft.com/en-us/windows/uwp/design/shell/tiles-and-notifications/toast-progress-bar for more info.
//
...
func invokeTemporaryScript(content string) error {
	filename := "469b79f6-6968-4cc2-bdbe-966da0378fd0.ps1"
